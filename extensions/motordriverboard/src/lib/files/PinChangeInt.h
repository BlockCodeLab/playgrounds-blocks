...
#if ((defined(NO_PORTA_PINCHANGES) && defined(NO_PORTB_PINCHANGES) &&          \
      defined(NO_PORTC_PINCHANGES)) ||                                         \
     (defined(NO_PORTA_PINCHANGES) && defined(NO_PORTB_PINCHANGES) &&          \
      defined(NO_PORTD_PINCHANGES)) ||                                         \
     (defined(NO_PORTA_PINCHANGES) && defined(NO_PORTC_PINCHANGES) &&          \
      defined(NO_PORTD_PINCHANGES)) ||                                         \
     (defined(NO_PORTB_PINCHANGES) && defined(NO_PORTC_PINCHANGES) &&          \
      defined(NO_PORTD_PINCHANGES)))
#define INLINE_PCINT inline
#endif
#endif
...
public:
  ...
  PCintPort(int index, int pcindex, volatile uint8_t &maskReg)
      : portInputReg(*portInputRegister(index)), portPCMask(maskReg),
        PCICRbit(1 << pcindex), portRisingPins(0), portFallingPins(0),
        firstPin(NULL)
#ifdef PINMODE
        ,
        intrCount(0)
  ...
  static int8_t attachInterrupt(uint8_t pin, PCIntvoidFuncPtr userFunc,
                                int mode);
...
protected:
  class PCintPin {
  public:
    PCintPin() : PCintFunc((PCIntvoidFuncPtr)NULL), mode(0) {}
...
//
    ...
    defined(__AVR_ATmega644__) // Sanguino, Mosquino uino bobino
                               // bonanafannafofino, me my momino...

#ifndef NO_PORTA_PINCHANGES
PCintPort portA = PCintPort(
    1, 0, PCMSK0); // port PA==1  (from Arduino.h, Arduino version 1.0)
#endif
#ifndef NO_PORTB_PINCHANGES
PCintPort portB = PCintPort(
    2, 1, PCMSK1); // port PB==2  (from Arduino.h, Arduino version 1.0)
#endif
#ifndef NO_PORTC_PINCHANGES
PCintPort portC = PCintPort(
    3, 2, PCMSK2); // port PC==3  (also in pins_arduino.c, Arduino version 022)
#endif
...
#ifndef NO_PORTB_PINCHANGES
PCintPort portB = PCintPort(
    2, 0, PCMSK0); // port PB==2  (from Arduino.h, Arduino version 1.0)
#endif
#ifndef NO_PORTC_PINCHANGES // note: no PORTC on MEGA
PCintPort portC = PCintPort(
    3, 1, PCMSK1); // port PC==3  (also in pins_arduino.c, Arduino version 022)
#endif
...
int8_t PCintPort::addPin(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc,
  ...
  tmp = firstPin;
  // Add to linked list, starting with firstPin. If pin already exists, just
  // enable.
  if (firstPin != NULL) {
    do {
  ...
  if (firstPin == NULL)
    ...
    tmp->next = p; // NOTE that tmp cannot be NULL.
 ...
 */
int8_t PCintPort::attachInterrupt(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc,
                                  int mode) {
  PCintPort *port;
  ...
  // Added by GreyGnome... must set the initial value of lastPinView for it to
  // be correct on the 1st interrupt.
  // ...but even then, how do you define "correct"?  Ultimately, the user must
  // specify (not provisioned for yet).
...
void PCintPort::detachInterrupt(uint8_t arduinoPin) {
  ...
  while (current) {
    if (current->mask == mask) { // found the target
      ...
      // Note that in the addPin() function, above, we do not define a new pin
      // if it was once already defined.
      // ... ...
      // Link the previous' next to the found next. Then remove the found.
      // if (prev != NULL) prev->next=current->next; // linked list skips over
      // current. else firstPin=current->next; // at the first pin; save the new
      // first pin
...
// get the pin states for the indicated port.
    ...
    PCintPin *p = firstPin;
    while (p) {
      // Trigger interrupt if the bit is high and it's set to trigger on mode
      // RISING or CHANGE Trigger interrupt if the bit is low and it's set to
      // trigger on mode FALLING or CHANGE
    ...
    if (PCIFR & PCICRbit)
      PCintPort::PCIFRbug = 1; // PCIFR & PCICRbit should ALWAYS be 0 here!
#endif
...
uint16_t getPCIntVersion() { return ((uint16_t)PCINT_VERSION); }
