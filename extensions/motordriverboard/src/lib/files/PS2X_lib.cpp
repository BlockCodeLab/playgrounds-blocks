...
static byte set_bytes_large[] = {0x01, 0x4F, 0x00, 0xFF, 0xFF,
                                 0x03, 0x00, 0x00, 0x00};
static byte exit_config[] = {0x01, 0x43, 0x00, 0x00, 0x5A,
                             0x5A, 0x5A, 0x5A, 0x5A};
static byte enable_rumble[] = {0x01, 0x4D, 0x00, 0x00, 0x01};
static byte type_read[] = {0x01, 0x45, 0x00, 0x5A, 0x5A,
                           0x5A, 0x5A, 0x5A, 0x5A};
...
boolean PS2X::read_gamepad(boolean motor1, byte motor2) {
  ...
  if (motor2 != 0x00)
    motor2 =
        map(motor2, 0, 255, 0x40, 0xFF); // noting below 40 will make it spin
