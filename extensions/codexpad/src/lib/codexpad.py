...
_MANUFACTURER_DATA_LENGTH = (
    len(_MANUFACTURER_HEADER) + _MANUFACTURER_FIRMWARE_VERSION_LENGTH + 4 + 1
)
_MANUFACTURER_DATA_UNPACK_FMT = (
    "<"
    + _MANUFACTURER_HEADER_UNPACK_FMT
    + _FIRMWARE_VERSION_UNPACK_FMT
    + _BUTTON_STATE_UNPACK_FMT
    + "B"
)
...
try:
    ...
    from blerepl import bleuart_handler

    register_irq_handler(bleuart_handler, None)
except ImportError:
    pass
...
class CodexPad:
    ...
    async def scan_and_connect_10(
        self, button_mask, scan_duration_ms=1000, connect_timeout_ms=5000
    ):
        for _ in range(10):
            try:
    ...
    async def scan_and_connect(
        ...
        async with aioble.scan(
            scan_duration_ms, interval_us=50000, window_us=20000, active=True
        ) as scanner:
            async for result in scanner:
                ...
                unpacked = struct.unpack(
                    _MANUFACTURER_DATA_UNPACK_FMT,
                    manufacturer_data[:_MANUFACTURER_DATA_LENGTH],
                )
                ...
                firmware_version = bytes(
                    unpacked[
                        len(_MANUFACTURER_HEADER) : len(_MANUFACTURER_HEADER)
                        + _MANUFACTURER_FIRMWARE_VERSION_LENGTH
                    ]
                )
                ...
                button_state = unpacked[
                    len(_MANUFACTURER_HEADER) + _MANUFACTURER_FIRMWARE_VERSION_LENGTH
                ]
                ...
                if device is None or result.rssi > rssi:
                    ...
                    # )
        if not device:
            raise CodexPadNotFoundError(
                f"No CodexPad device found with button mask 0x{button_mask:08X}"
            )
    ...
    async def connect(self, bluetooth_device_address, timeout_ms):
        await self._connect(
            aioble.Device(aioble.ADDR_PUBLIC, bluetooth_device_address), timeout_ms
        )
    ...
    async def _connect(self, device, timeout_ms):
        ...
        try:
            self._connection = await device.connect(
                timeout_ms=timeout_ms, scan_duration_ms=1000
            )

            gap_service = await self._connection.service(_GAP_SERVICE_UUID)
            device_name_characteristic = await gap_service.characteristic(
                _GAP_DEVICE_NAME_UUID
            )
            self._remote_device_name = (await device_name_characteristic.read()).decode(
                "utf-8"
            )

            device_info_service = await self._connection.service(
                _DEVICE_INFO_SERVICE_UUID
            )
            model_number_characteristic = await device_info_service.characteristic(
                _MODEL_NUMBER_STRING_UUID
            )
            self._remote_model_number = (
                await model_number_characteristic.read()
            ).decode("utf-8")

            firmware_revision_characteristic = await device_info_service.characteristic(
                _FIRMWARE_REVISION_STRING_UUID
            )
            self._remote_firmware_version = tuple(
                struct.unpack(
                    _FIRMWARE_VERSION_UNPACK_FMT,
                    (await firmware_revision_characteristic.read()),
                )
            )

            tx_power_service = await self._connection.service(_TX_POWER_SERVICE_UUID)
            tx_power_characteristic = await tx_power_service.characteristic(
                _TX_POWER_CHARACTERISTIC_UUID
            )
            self._tx_power_characteristic = tx_power_characteristic

            inputs_service = await self._connection.service(_INPUTS_SERVICE_UUID)
            inputs_characteristic = await inputs_service.characteristic(
                _INPUTS_CHARACTERISTIC_UUID
            )
    ...
    async def set_remote_tx_power(self, tx_power):
        ...
        await self._tx_power_characteristic.write(
            struct.pack("<b", tx_power), timeout_ms=5000
        )
    ...
    async def update(self):
        ...
        if (
            ...
            or self._inputs_characteristic is None
            or not self._connection.is_connected()
        ):
            return

        try:
            self._current_inputs.parse_and_set(
                await self._inputs_characteristic.notified(timeout_ms=1)
            )
        except Exception:
            pass

    def pressed(self, button):
        return ((self._prev_inputs.button_states & button) == 0) and (
            (self._current_inputs.button_states & button) != 0
        )
    ...
    def _has_axis_value_changed_significantly(
        self, prev_value, current_value, threshold
    ):
        return prev_value != current_value and (
            current_value == 0
            or current_value == 255
            or abs(current_value - prev_value) >= threshold
        )

    def has_axis_value_changed(self, axis, threshold):
        return self._has_axis_value_changed_significantly(
            self._prev_inputs.axis_values[axis],
            self._current_inputs.axis_values[axis],
            threshold,
        )
