...
Mpu6050::Mpu6050(uint8_t device_i2c_address)
    : i2c_device_(device_i2c_address) {}
...
bool Mpu6050::UpdateMotionInfo() {
  ...
  gravity_.x =
      2 * (quaternion_.x * quaternion_.z - quaternion_.w * quaternion_.y);
  gravity_.y =
      2 * (quaternion_.w * quaternion_.x + quaternion_.y * quaternion_.z);
  gravity_.z = quaternion_.w * quaternion_.w - quaternion_.x * quaternion_.x -
...
Mpu6050::Euler Mpu6050::GetEuler() const {
  ...
  euler.yaw = atan2(2 * quaternion_.x * quaternion_.y -
                        2 * quaternion_.w * quaternion_.z,
                    2 * quaternion_.w * quaternion_.w +
                        2 * quaternion_.x * quaternion_.x - 1) *
              180 / M_PI;
  euler.pitch = atan2(gravity_.x,
                      sqrt(gravity_.y * gravity_.y + gravity_.z * gravity_.z)) *
                180 / M_PI;
...
Mpu6050::Gravity Mpu6050::GetGravity() const { return gravity_; }
...
bool Mpu6050::GetCurrentFifoPacket(uint8_t *data, const uint8_t length) {
  ...
  do {
    if ((fifo_count = GetFifoCount()) > length) {
      if (fifo_count >
          200) { // if you waited to get the FIFO buffer to > 200 bytes
        ...
        i2c_device_.WriteBit(MPU6050_RA_USER_CTRL, 2,
                             1); // Fixes any overflow corruption
        fifo_count = 0;
        while (!(fifo_count = GetFifoCount()) &&
               ((micros() - break_timer) <= MPU6050_FIFO_DEFAULT_TIMEOUT))
          ;    // Get Next New Packet
      } else { // We have more than 1 packet but less than 200 bytes of data in
               // the FIFO Buffer
        uint8_t Trash[BUFFER_LENGTH];
        while ((fifo_count = GetFifoCount()) >
               length) { // Test each time just in case the MPU is writing to
                         // the FIFO Buffer
          fifo_count = fifo_count - length; // Save the last packet
          uint16_t RemoveBytes;
          while (fifo_count) { // fifo count will reach zero so this is safe
            RemoveBytes = (fifo_count < BUFFER_LENGTH)
                              ? fifo_count
                              : BUFFER_LENGTH; // Buffer Length is
                                               // different than the
                                               // packet length this
                                               // will efficiently
                                               // clear the buffer
