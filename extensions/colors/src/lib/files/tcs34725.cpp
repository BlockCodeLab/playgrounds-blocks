...
TCS34725::TCS34725() { _tcs34725Initialised = false; }
...
void TCS34725::getRGBC(uint16_t *r, uint16_t *g, uint16_t *b, uint16_t *c,
  ...
  if (c)
    *c = readRegWord(TCS34725_CDATAL);
    ...
    readRegWord(TCS34725_CDATAL);

  if (r)
    *r = readRegWord(TCS34725_RDATAL);
  else
    readRegWord(TCS34725_RDATAL);

  if (g)
    *g = readRegWord(TCS34725_GDATAL);
  else
    readRegWord(TCS34725_GDATAL);

  if (b)
    *b = readRegWord(TCS34725_BDATAL);
  else
    readRegWord(TCS34725_BDATAL);
...
uint16_t TCS34725::calculateColorTemperature(uint16_t r, uint16_t g,
  ...
  cct = (449.0F * _powf(n, 3)) + (3525.0F * _powf(n, 2)) + (6823.3F * n) +
        5520.33F;
