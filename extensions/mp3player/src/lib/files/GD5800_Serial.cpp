...
void GD5800_Serial::play() { this->sendCommand(0x01); }
...
void GD5800_Serial::pause() { this->sendCommand(0x02); }

void GD5800_Serial::next() { this->sendCommand(0x03); }

void GD5800_Serial::prev() { this->sendCommand(0x04); }

void GD5800_Serial::fastReverse() { this->sendCommand(0x0B); }

void GD5800_Serial::fastForward() { this->sendCommand(0x0A); }
...
void GD5800_Serial::volumeUp() { this->sendCommand(0x05); }

void GD5800_Serial::volumeDn() { this->sendCommand(0x06); }
...
byte GD5800_Serial::getStatus() {
  ...
  } while (statTotal != 1 * MP3_STATUS_CHECKS_IN_AGREEMENT &&
           statTotal != 2 * MP3_STATUS_CHECKS_IN_AGREEMENT);

  return statTotal / MP3_STATUS_CHECKS_IN_AGREEMENT;
}

byte GD5800_Serial::getVolume() {
  return this->sendCommandWithUnsignedIntResponse(0x11);
}

byte GD5800_Serial::getEqualizer() {
  return this->sendCommandWithUnsignedIntResponse(0x44);
}

byte GD5800_Serial::getLoopMode() {
  return this->sendCommandWithUnsignedIntResponse(0x13);
}
...
void GD5800_Serial::sendCommand(byte command, byte arg1, byte arg2,
  ...
  // Command structure
  // [7E][number bytes following including command and terminator][command
  // byte][?arg1][?arg2][EF]
  ...
  // The device appears to send some sort of status information (namely "STOP"
  // when it stops playing) just discard this right before we send the command
  while (this->waitUntilAvailable(10))
    this->read();
...
size_t GD5800_Serial::readBytesUntilAndIncluding(char terminator, char *buffer,
                                                 size_t length,
                                                 byte maxOneLineOnly) {
  if (length < 1)
    return 0;
