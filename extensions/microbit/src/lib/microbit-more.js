// fork https://github.com/microbit-more/mbit-more-v2/blob/master/src/vm/extensions/block/microbit-more.js
...
export class MicrobitMore {
  ...
  constructor(runtime) {
    ...
    if (this.runtime) {
      this.runtime.on('stop', () => this.stopTone());
    }
  ...
  startUpdater() {
    ...
    this.updateState()
      ...
      .finally(() => {
        this.updater = setTimeout(() => this.startUpdater(), this.microbitUpdateInterval);
      });
  }
      ...
      [
        {
          ...
          message: new Uint8Array([Math.min(255, Math.max(0, delay) / 10), ...textData]),
        },
      ],
      util,
    );
    ...
    const cmdSet = [
      {
        ...
        message: new Uint8Array([...matrix[0], ...matrix[1], ...matrix[2]]),
      },
      {
  ...
  readAnalogIn(pinIndex, util) {
    ...
    return new Promise((resolve) =>
      this._ble.read(MM_SERVICE.ID, MM_SERVICE.ANALOG_IN_CH[pinIndex], false).then((result) => {
  ...
  updateState() {
    ...
    return new Promise((resolve) => {
      this._ble.read(MM_SERVICE.ID, MM_SERVICE.STATE_CH, false).then((result) => {
        window.clearTimeout(this.bleBusyTimeoutID);
        this.bleBusy = false;
  ...
  updateMotion() {
    ...
    return new Promise((resolve) => {
      this._ble.read(MM_SERVICE.ID, MM_SERVICE.MOTION_CH, false).then((result) => {
        window.clearTimeout(this.bleBusyTimeoutID);
        this.bleBusy = false;
        if (!result) return resolve(this);
        const data = result.message;
  ...
  readAcceleration(axis) {
    ...
    if (axis === AxisSymbol.Absolute) {
      return Math.round(Math.sqrt(this.acceleration.x ** 2 + this.acceleration.y ** 2 + this.acceleration.z ** 2));
    }
  ...
  readMagneticForce(axis) {
    ...
    if (axis === AxisSymbol.Absolute) {
      return Math.round(Math.sqrt(this.magneticForce.x ** 2 + this.magneticForce.y ** 2 + this.magneticForce.z ** 2));
    }
   ...
   * @param {object} server - the web bluetooth gatt server object.
   */
  connect(server) {
    this._ble = new BLE(server);
    this._onConnect();
  }
   ...
   */
  sendCommand(command) {
    const data = new Uint8Array([command.id, ...command.message]);
    return new Promise((resolve) => {
      this._ble.write(MM_SERVICE.ID, MM_SERVICE.COMMAND_CH, data, null, false);
  ...
  sendCommandSet(commands, util) {
    ...
    return new Promise((resolve) => {
      commands
        .reduce((acc, cur) => acc.then(() => this.sendCommand(cur)), Promise.resolve())
        .then(() => {
          window.clearTimeout(this.bleBusyTimeoutID);
        })
        .catch((err) => {
    ...
    this._ble
      ...
      .then((result) => {
        ...
        if (this.hardware === MbitMoreHardwareVersion.MICROBIT_V1) {
          ...
          this._ble.startNotifications(MM_SERVICE.ID, MM_SERVICE.MESSAGE_CH, this.onNotify);
  ...
  sendData(label, content, util) {
    const labelData = new Array(8).fill().map((_value, index) => label.charCodeAt(index));
      ...
      [
        {
          ...
          message: new Uint8Array([...labelData, ...contentData]),
        },
      ],
      util,
    );
