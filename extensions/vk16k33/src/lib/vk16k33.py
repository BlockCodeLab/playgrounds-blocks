...
class Vk16k33(X16k33):
    ...
    def show_colon(self, show: bool) -> None:
        ...
        """
        self[Vk16k33._COLON_POSITION << 1] = 0xFF if show else 0x00
        self.write()
    ...
    def show_digit_number(self, position: int, number: int, dot: bool = False) -> None:
        ...
        """
        self[self._digit_buffer_position(position)] = Vk16k33._NUMBER_TABLE[number] | (
            dot << 7
        )
        self.write()
    ...
    def show_number(
        ...
        numeric_digits = Vk16k33._DIGIT_NUMBER
        ...
        position = Vk16k33._DIGIT_NUMBER - 1

        index = 0
        while (
            index <= fractional_part_digits
            or display_number > 0
            or position == Vk16k33._DIGIT_NUMBER - 1
        ):
            display_decimal = (
                fractional_part_digits != 0 and index == fractional_part_digits
            )
            self[self._digit_buffer_position(position)] = Vk16k33._NUMBER_TABLE[
    ...
    def _digit_buffer_position(self, digit_position) -> int:
        return (
            digit_position
            if Vk16k33._COLON_POSITION < 0 or digit_position < Vk16k33._COLON_POSITION
