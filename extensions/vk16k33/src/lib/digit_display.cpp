...
DigitDisplay::DigitDisplay(const uint8_t device_i2c_address)
...
void DigitDisplay::ShowDigitNumber(uint8_t index, const uint8_t number,
                                   const bool dot) {
...
void DigitDisplay::ShowNumber(double number, Base base,
                              int fractional_part_digits) {
  if (fractional_part_digits < 0) {
    fractional_part_digits = getDecimalPlaces(number);
  }
  ...
  for (uint8_t i = 0; i <= fractional_part_digits || display_number > 0 ||
                      position == DIGIT_NUMBER;
       i++) {
    const bool display_decimal =
        (fractional_part_digits != 0 && i == fractional_part_digits);
    display_buffer_[position--] =
        g_number_table[display_number % base] | (display_decimal << 7);
